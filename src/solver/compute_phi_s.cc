...
vector<vector<int> > result;
	...
	vector<int> ei;
		...
		for (unsigned int k=0;k!=i&&k<extensions.size();k++){
		vector<int> vect = extensions[k];
...
vector<vector<int> > cnfPart2 = transformationTseytin(part2);
...
part3.push_back({-xs});
vector<vector<int> > cnfPart3 = transformationTseytin(part3);
